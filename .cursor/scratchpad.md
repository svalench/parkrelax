...
Карта маршрутов: `frontend/public/park-relax-map-light.html` с `?embed=1` (только карта, fitBounds + invalidateSize), iframe в `About.tsx` под сеткой; корневой `frontend/park-relax-map-light.html` удалён. `npm run build` и eslint для `About.tsx` — ок.
